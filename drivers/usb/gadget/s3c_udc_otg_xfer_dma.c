...
//	dma_cache_maint(p, length, DMA_FROM_DEVICE);
	...
	//length = ep->ep.maxpacket + 1; // HACK!
	writel(the_controller->dma_addr[ep_index(ep)+1], S3C_UDC_OTG_DOEPDMA(ep_num));
...
//	dma_cache_maint(p, length, DMA_TO_DEVICE);
	...
	writel(the_controller->dma_addr[ep_index(ep)+1], S3C_UDC_OTG_DIEPDMA(ep_num));
...
static void complete_rx(struct s3c_udc *dev, u8 ep_num)
{
	...
	if (is_short || req->req.actual == req->req.length) {
		if (ep_num == EP0_CON && dev->ep0state == DATA_STATE_RECV) {
			...
			s3c_udc_ep0_zlp(dev);
			dev->ep0state = WAIT_FOR_IN_COMPLETE; /* packet will be completed in complete_tx() */
		} else {
			done(ep, req, 0);
...
static void process_ep_out_intr(struct s3c_udc * dev)
{
	...
	while(ep_intr) {
		if (ep_intr & 0x1) {
			...
			if (ep_num == 0 ) {
				if (ep_intr_status & TRANSFER_DONE) {
					if (dev->ep0state != WAIT_FOR_OUT_COMPLETE)
...
static int write_fifo_ep0(struct s3c_ep *ep, struct s3c_request *req)
{
	...
	if (likely(count != max))
		...
		if (likely(req->req.length != req->req.actual + count) || req->req.zero)
			is_last = 0;
		else
			is_last = 1;
	}
	...
	DEBUG_EP0("%s: wrote %s %d bytes%s %d left %p\n", __func__,
		  ep->ep.name, count,
		  is_last ? "/L" : "", req->req.length - req->req.actual - count, req);
