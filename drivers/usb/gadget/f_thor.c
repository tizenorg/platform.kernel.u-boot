				   ...
				   int *cnt)
{
	...
	/*
	 * Files smaller than THOR_STORE_UNIT_SIZE (now 32 MiB) store at
	 * once at medium.
	 * The packet response is sent on the purpose after successful data
	 * chunk write. Here is a room for improvement when asynchronous write
	 * is performed.
	 */
	...
	/*
	 * Calculate number of left data - those which are smaller than
	 * packet size and are supposed to come from PC
	 */
...
static int download_tail(long long int left, int cnt)
{
	...
	if (left) {
		...
		if (ret) {
			error("DFU write failed [%d]: left: %llu\n", ret, left);
			return ret;
		}
	}
...
static long long int process_rqt_download(const struct rqt_box *rqt)
{
	...
	case RQT_DL_EXIT:
		...
		break;
	default:
		error("Operation not supported: %d\n", rqt->rqt_data);
		ret = -ENOTSUPP;
	}
...
void thor_tx_data(char *data, int len)
{
	struct thor_dev *dev = thor_func->dev;
...
thor_func_setup(struct usb_function *f, const struct usb_ctrlrequest *ctrl)
{
	...
	default:
		error("thor_setup: unknown request: %d\n", ctrl->bRequest);
	}
...
/* Specific to the THOR_THOR protocol */
void thor_set_dma(void *addr, int len)
{
	struct thor_dev *dev = thor_func->dev;
