		...
		unsigned int data0, unsigned int data1)
{
	...
	for (data_cnt = 0; data_cnt < data1; data_cnt += 4) {
		...
		if ((data1 - data_cnt) < 4) {
			...
			udebug("count = 3 payload = %x, %x %x %x\n",
				payload, *(u8 *)(data0 + data_cnt),
				*(u8 *)(data0 + (data_cnt + 1)),
				*(u8 *)(data0 + (data_cnt + 2)));
		...
		/* send 4bytes per one time. */
			...
			udebug("count = 4 payload = %x, %x %x %x %x\n",
				payload, *(u8 *)(data0 + data_cnt),
				*(u8 *)(data0 + (data_cnt + 1)),
				*(u8 *)(data0 + (data_cnt + 2)),
				*(u8 *)(data0 + (data_cnt + 3)));
	...
	unsigned int scaler)
{
	...
	if (dfin_pll < DFIN_PLL_MIN_HZ || dfin_pll > DFIN_PLL_MAX_HZ) {
		udebug("fin_pll range should be 6MHz ~ 12MHz\n");
	...
	if (dfvco < DFVCO_MIN_HZ || dfvco > DFVCO_MAX_HZ)
		udebug("fvco range should be 500MHz ~ 1000MHz\n");
	...
	struct mipi_dsim_config *dsim_config)
{
	...
	udebug("lcd panel ==> width = %d, height = %d\n",
...
int s5p_mipi_dsi_init_link(struct mipi_dsim_device *dsim)
{
	...
	case DSIM_STATE_INIT:
		...
		if (time_out != 0) {
			udebug("DSI Master driver has been completed.\n");
