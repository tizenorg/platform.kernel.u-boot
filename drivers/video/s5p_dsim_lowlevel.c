...
void s5p_dsim_set_main_disp_resol(unsigned int dsim_base,
	unsigned short vert_resol, unsigned short hori_resol)
{
	unsigned int reg;

	/* standby should be set after configuration so set to not ready*/
	reg = (readl(dsim_base + S5P_DSIM_MDRESOL)) &
		~(DSIM_MDRESOL_MAIN_STAND_BY);
	writel(reg, dsim_base + S5P_DSIM_MDRESOL);

	reg &= ~(0x7ff << 16) & ~(0x7ff << 0);
	reg |= DSIM_MDRESOL_MAIN_VRESOL(vert_resol) |
	    DSIM_MDRESOL_MAIN_HRESOL(hori_resol);

	reg |= DSIM_MDRESOL_MAIN_STAND_BY;
	writel(reg, dsim_base + S5P_DSIM_MDRESOL);
}

void s5p_dsim_set_main_disp_vporch(unsigned int dsim_base,
...
void s5p_dsim_set_main_disp_sync_area(unsigned int dsim_base,
...
void s5p_dsim_init_config(unsigned int dsim_base,
...
void s5p_dsim_enable_lane(unsigned int dsim_base, unsigned char lane,
...
void s5p_dsim_force_dphy_stop_state(unsigned int dsim_base,
	unsigned char enable)
{
...
unsigned char s5p_dsim_is_lane_state(unsigned int dsim_base,
	unsigned char lane)
{
	...
	if ((lane & DSIM_LANE_ALL) > DSIM_LANE_CLOCK) { /* all lane state */
		...
		if (reg & (lane << 4))
			...
			udebug("data lane state is unknown.\n");
			return -1;
		}
...
void s5p_dsim_set_stop_state_counter(unsigned int dsim_base,
...
{
	unsigned int reg = readl(dsim_base + S5P_DSIM_INTSRC);

	reg |= int_src;

	writel(reg, dsim_base + S5P_DSIM_INTSRC);
}
...
unsigned int _s5p_dsim_get_frame_done_status(unsigned int dsim_base)
{
	unsigned int reg = readl(dsim_base + S5P_DSIM_INTSRC);

	return (reg & DSIM_INTSRC_FRAME_DONE) ? 1 : 0;
}

void _s5p_dsim_clear_frame_done(unsigned int dsim_base)
{
	unsigned int reg = readl(dsim_base + S5P_DSIM_INTSRC);

	writel(reg | DSIM_INTSRC_FRAME_DONE, dsim_base +
		S5P_DSIM_INTSRC);
}
