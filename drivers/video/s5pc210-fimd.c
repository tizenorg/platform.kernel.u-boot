...
static void s5pc_fimd_set_clock(void)
{
	...
	temp = readl(&clk->src_lcd0);
	writel((temp & ~0xf00000) | 0x600000, &clk->src_lcd0);
	temp = 0;

	/* set fimd ratio to 3. */
	temp = readl(&clk->div_lcd0);
	writel((temp & ~0xf00000) | 0x200000, &clk->div_lcd0);
	temp = 0;
