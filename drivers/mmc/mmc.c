...
int mmc_change_freq(struct mmc *mmc)
{
	...
	if (err)
		return err;
#if 0
	/* don't check this register to determine high capacity
	   it also contain low capacity */
...
int mmc_startup(struct mmc *mmc)
{
	...
	if (!IS_SD(mmc) && (mmc->version >= MMC_VERSION_4)) {
		...
		err = mmc_send_ext_csd(mmc, ext_csd);
		if (!err && mmc->high_capacity && (ext_csd[EXT_CSD_REV] >= 2)) {
...
void print_mmc_devices(char separator)
{
	...
	list_for_each(entry, &mmc_devices) {
		...
		/* we are initiazlied mmc dev0 only */
		if(m->block_dev.dev == 0)
			mmc_init(m);
#endif
