...
struct animation_image {
	...
	struct bmp_image **clc_img;
};

static int last_bar_cnt;

/* Common bar */
static struct bmp_image *bmp_bar_state;

/* Low state bar */
static struct bmp_image *bmp_bar_red_state;

/* Charge animation screen */
static struct bmp_image *charge_bmp;
static struct bmp_image *charge_clc_bmp;

/* Indicator images */
static struct bmp_image *indicator_bmp;
static struct bmp_image *indicator_clc_horiz_bmp;
static struct bmp_image *indicator_clc_vert_bmp;
...
{
	static struct bmp_image *battery;
	void *frame_bitmap;
	...
	if (!battery)
		battery = (struct bmp_image *)battery_frame;
...
static void prepare_battery_state(void)
{
	...
	if (!bmp_bar_state)
		bmp_bar_state = gunzip_bmp((long unsigned int)
					   ...
					   NULL, &bar_bitmap);
	if (!bmp_bar_state)
		bmp_bar_state = (struct bmp_image *)battery_bar;
	...
	if (!bmp_bar_red_state)
		bmp_bar_red_state = gunzip_bmp((long unsigned int)
					       ...
					       NULL, &bar_red_bitmap);
	if (!bmp_bar_red_state)
		bmp_bar_red_state = (struct bmp_image *)battery_bar_red;
}
...
static void prepare_charge_screen(void)
{
	...
	if (!charge_bmp)
		charge_bmp = gunzip_bmp((long unsigned int)charge_screen,
					...
					&charge_bitmap);
	if (!charge_bmp)
		charge_bmp = (struct bmp_image *)charge_screen;
	...
	if (!charge_clc_bmp)
		charge_clc_bmp = gunzip_bmp((long unsigned)charge_screen_clc,
					    ...
					    &charge_clc_bitmap);
	if (!charge_clc_bmp)
		charge_clc_bmp = (struct bmp_image *)charge_screen_clc;
}
...
static void prepare_charge_animation(void)
{
	...
	if (!indicator_bmp)
		indicator_bmp = gunzip_bmp((long unsigned int)indicator, NULL,
					   &indicator_bitmap);
	if (!indicator_bmp)
		indicator_bmp = (struct bmp_image *)indicator;
	...
	if (!indicator_clc_horiz_bmp)
		indicator_clc_horiz_bmp = gunzip_bmp((long unsigned int)
						     ...
						     &clc_horiz_bitmap);
	if (!indicator_clc_horiz_bmp)
		indicator_clc_horiz_bmp = (struct bmp_image *)indicator_clc_horiz;
	...
	if (!indicator_clc_vert_bmp)
		indicator_clc_vert_bmp = gunzip_bmp((long unsigned int)
						    ...
						    &clc_vert_bitmap);
	if (!indicator_clc_vert_bmp)
		indicator_clc_vert_bmp = (struct bmp_image *)indicator_clc_vert;
}
