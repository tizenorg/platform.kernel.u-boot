...
{
	phy_base = DRIME3_PHY_BASE;
	otg_base = DRIME3_OTG_BASE;
}
...
{
	s5pc1xx_phy_write_reg(0x1, OTG_SLEEP_CTRL);
	s5pc1xx_phy_write_reg(0x0, OTG_UTMI_ETC);
	s5pc1xx_phy_write_reg(0x2c, OTG_PHY_CLK);
	s5pc1xx_phy_write_reg(0x12330f, OTG_PHY_TUNE);

	s5pc1xx_phy_write_reg(0x0, OTG_RST_CTRL);
	udelay(20);
	s5pc1xx_phy_write_reg(0x4, OTG_RST_CTRL);
	s5pc1xx_phy_write_reg(0x5, OTG_RST_CTRL);
	udelay(20);
}
...
int s5p_usbctl_init(void)
{
	...
	/* global clock enable */
	reg = readl(DRIME3_CLOCK_BASE + 0x8);
	reg &= ~(1 << 2); /* USB_CLK_CON ON */
	writel(reg, DRIME3_CLOCK_BASE + 0x8);

	/* USB OTG Clock Gating enable */
	writel(0x1, DRIME3_SYSTEM_BASE + 0x424);
