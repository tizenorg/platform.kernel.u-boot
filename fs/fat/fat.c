	      ...
	      unsigned long offset, unsigned long maxsize)
{
	...
	__u32 endclust, newclust, offset_clust;
	...
	if (offset) {
		offset_clust = offset / bytesperclust;
	...
	debug("File offset: %ld bytes, size: ", offset);
	debug("%ld bytes\n", filesize);
...
long
do_fat_read (const char *filename, void *buffer, unsigned long offset,
...
rootdir_done:
	...
	while (isdir) {
		...
		if (get_dentfromdir(mydata, startsect, subname, dentptr,
				     isdir ? 0 : dols) == NULL) {
			if (dols && !isdir)
				return (long)FAT2CPU32(dentptr->size);
			return -1;
		}
	...
	ret = get_contents(mydata, dentptr, buffer, offset, maxsize);
...
long file_fat_read (const char *filename, void *buffer,
	unsigned long offset, unsigned long maxsize)
{
	printf("reading %s from %d size %d\n", filename, offset, maxsize);
	return do_fat_read(filename, buffer, offset, maxsize, LS_NO);
}
