...
errcode_t ext2fs_allocate_tables(ext2_filsys fs)
{
	errcode_t	retval;
	dgrp_t		i;
	//unsigned char *zero_buffer = (unsigned char *)xzalloc(fs->blocksize);
	...
	for (i = 0; i < fs->group_desc_count; i++) {		
		...
		if (retval)
			return retval;
#if 0
	/*	Added by SISO: Not in mkfs.ext4 utility */
	/* Added to initialize block bitmaps and inode bitmaps to 0 */	
		
		put_ext4(dev_desc, (uint64_t)(fs->group_desc[i].bg_block_bitmap * fs->blocksize), 
							zero_buffer,(uint32_t)(fs->blocksize));
		put_ext4(dev_desc, (uint64_t)(fs->group_desc[i].bg_inode_bitmap * fs->blocksize), 
							zero_buffer,(uint32_t)(fs->blocksize));
#endif
	}
	return 0;
