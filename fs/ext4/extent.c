...
//uma
		...
		retval= ext2fs_devread(blk * factor , 0, handle->fs->blocksize,	newpath->buf);
		retval=0; //uma hack
			if (retval)
				return retval;
		}
...
#endif	
	put_ext4(dev_desc, (uint64_t)(blk* handle->fs->blocksize),handle->path[handle->level].buf,(uint32_t) handle->fs->blocksize);
	retval=0;
...
#endif		
		put_ext4(dev_desc, (uint64_t)(new_node_pblk* handle->fs->blocksize),block_buf,(uint32_t) handle->fs->blocksize);
		retval=0;
