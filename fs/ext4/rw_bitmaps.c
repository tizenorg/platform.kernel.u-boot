	...
	for (i = 0; i < fs->group_desc_count; i++) {
		...
		if (blk) {
			...
			retval = io_channel_write_blk(fs->io, blk, 1,
						      block_buf);
#endif
			put_ext4(dev_desc, (uint64_t)(blk* fs->blocksize), block_buf,(uint32_t) fs->blocksize);
			retval=0;
	...
	skip_block_bitmap:
		...
		//uma
			retval = io_channel_write_blk(fs->io, blk, 1,
						      inode_buf);
#endif
		put_ext4(dev_desc, (uint64_t)(blk* fs->blocksize), inode_buf,(uint32_t) fs->blocksize);
		retval=0;
...
static errcode_t read_bitmaps(ext2_filsys fs, int do_inode, int do_block)
{
	...
	ext2_ino_t ino_itr = 1;
	ext2_ino_t ino_cnt;
	int factor = fs->blocksize/512; //uma 
	EXT2_CHECK_MAGIC(fs, EXT2_ET_MAGIC_EXT2FS_FILSYS);
	...
	if (fs->flags & EXT2_FLAG_IMAGE_FILE) {
		...
		while (inode_nbytes > 0) {
			...
			retval = io_channel_read_blk(fs->image_io, blk++,
						     1, inode_bitmap);
#endif	
			retval= ext2fs_devread( (blk * factor) , 0, fs->blocksize,inode_bitmap);
			blk++;
			retval=0; //uma hack
		...
		while (block_nbytes > 0) {
			...
			retval = io_channel_read_blk(fs->image_io, blk++,
						     1, block_bitmap);
#endif
			retval= ext2fs_devread( (blk * factor) , 0, fs->blocksize,block_bitmap);
			blk++;
			retval=0;
			if (retval)
				goto cleanup;
...
//uma
...
#endif				
				//uma ***retval= ext2fs_devread( (blk * factor) , 0, (int)((-block_nbytes) *fs->blocksize),block_bitmap);
				/*UMA: TODO fix the nelow bug */
				retval= ext2fs_devread( (blk * factor) , 0, (int)((block_nbytes) *fs->blocksize),block_bitmap);
				retval=0; //uma hack
		...
		if (inode_bitmap) {
			...
			//uma ***retval= ext2fs_devread( (blk * factor) , 0, (int)((-inode_nbytes) *fs->blocksize),inode_bitmap);
			/*UMA: TODO fix the nelow bug */
			retval= ext2fs_devread( (blk * factor) , 0, (int)((inode_nbytes) *fs->blocksize),inode_bitmap);
			retval=0; //uma hack
