					  ...
					  struct ext2_super_block *super)
{
	...
	//uma if (!fs->io->manager->write_byte || !fs->orig_super) {
	fallback:
	
		//uma io_channel_set_blksize(fs->io, SUPERBLOCK_OFFSET);
#if 0
		//uma
		retval = io_channel_write_blk(fs->io, 1, -SUPERBLOCK_SIZE,
					      super);
#endif		
		//uma put_ext4(dev_desc, (uint64_t)(1 * fs->blocksize),super,(uint32_t)(SUPERBLOCK_SIZE));		
		put_ext4(dev_desc, (uint64_t)(1 * 1024),super,(uint32_t)(SUPERBLOCK_SIZE));
		retval=0;
		//uma io_channel_set_blksize(fs->io, fs->blocksize);
		return retval;
	//}
...
//uma
		retval = io_channel_write_byte(fs->io,
			       SUPERBLOCK_OFFSET + (2 * write_idx), size,
					       new_super + write_idx);
#endif		
		put_ext4(dev_desc, (uint64_t)(SUPERBLOCK_OFFSET + (2 * write_idx)),new_super + write_idx,(uint32_t)(size));
		retval=0;
				    ...
				    struct ext2_super_block *super_shadow)
{
	...
	put_ext4(dev_desc, (uint64_t)(group_block * fs->blocksize), super_shadow ,(uint32_t)(SUPERBLOCK_SIZE));
	retval=0;
	return retval;
	...
	*super_shadow = *fs->super;
	...
	for (i = 0; i < fs->group_desc_count; i++) {
...
#endif			
			put_ext4(dev_desc, (uint64_t)(old_desc_blk * fs->blocksize), group_ptr,(uint32_t)(fs->blocksize*old_desc_blocks));
			retval=0;
		...
		if (new_desc_blk) {
			...
			put_ext4(dev_desc, (uint64_t)(new_desc_blk * fs->blocksize), group_ptr + (meta_bg*fs->blocksize),(uint32_t)(fs->blocksize));
					retval=0;
			if (retval)
				goto errout;
		}
...
#if 0
	printf("printing uuid in ubooot @@@@@@@@@@@@ \n");
	for(i=0;i<16;i++)
	{
		printf("%u\t",fs->super->s_uuid[i]);
	}
	printf("\n");
	printf("printing uuid in ubooot end @@@@@@@@@@@@ \n");
	printf("#########printing checksum info in uboot########\n");
	for (i=0; i < fs->group_desc_count; i++) {
		printf("======================\n");
		...
		printf("bg_reserved[0]=%u\n",fs->group_desc[i].bg_reserved[0]);
		printf("bg_reserved[1]=%u\n",fs->group_desc[i].bg_reserved[1]);
		printf("bg_itable_unused=%u\n",fs->group_desc[i].bg_itable_unused);
		printf("bg_checksum = %u\n",fs->group_desc[i].bg_checksum);
	};
	printf("#########printing checksum info in uboot finished ######## \n");
#endif
